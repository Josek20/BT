...
X = Float32.(reshape(collect(0.001f0:0.067f0:1.5f0), 1, :))
...
function makeplotFinall(model,name)
    p1 = Plots.plot(0.001f0:0.013f0:3f0, x->model([x])[1],line=(3),label=(name))
    Plots.plot!(p1, 0.001f0:0.013f0:3, f, ylims=range,line=(:dash,2),label="Truth",legend=:bottomright)
...
function makeplot(model)	
    p1 = plot(0.001f0:0.013f0:2f0, x->model([x])[1],line=(3))
    plot!(p1, 0.001f0:0.013f0:2f0, f, ylims=range,line=(:dash,2),legend=:bottomright)
...
function makePlotBigFinall(model,name,lines)
    Plots.plot!(0.001f0:0.013f0:3f0, x->model([x])[1],line=(3,lines),label=(name))
end

Plots.scatter(vec(X), vec(Y),label="Data")
Plots.plot!(0.001f0:0.013f0:3, f, ylims=range,line=(2),label="Truth",legend=:bottomright)
...
function modelsError(model,name)
	Xv = Float32.(reshape(collect(0.001f0:0.067f0:3f0), 1, :))
	Yv = f.(Xv)
